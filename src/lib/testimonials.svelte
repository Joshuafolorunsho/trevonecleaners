<script>
	import josephEmmanuel from './assets/joseph-emmanuel.png';
	import obadinaSamuel from './assets/obadina-samuel.png';
	import sarahAfolayan from './assets/sarah-afolayan.png';

	const testimonials = [
		{
			name: 'Joseph Emmanuel',
			imgUrl: josephEmmanuel,
			message:
...
<section class="flex flex-wrap gap-5 mt-12 justify-center">
	{#each testimonials as testimonial}
		<div class="flex-1 bg-white rounded-2xl px-8 py-3 text-center">
			<svg
				class="mx-auto my-6"
				width="69"
				height="67"
			...
			<p class="text-gray-950 font-semibold">{testimonial.message}</p>
			<div class="w-24 h-24 mx-auto my-2">
				<img src={testimonial.imgUrl} alt="" class="rounded-3xl" />
			</div>
			<h2 class="font-semibold text-lg">{testimonial.name}</h2>
			<p class="text-gray-750">{testimonial.company}</p>
		</div>
	{/each}
</section>
